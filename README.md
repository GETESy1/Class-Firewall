...
用**任务计划程序**建一个名为 `ClassFirewallAutoStart` 的任务：登录时触发，以最高权限运行，
启动命令是 `<exe> -silent`。**这是 Windows 上唯一能免 UAC 静默提权的官方途径** ——
程序必须提权（改网卡 DNS、写浏览器策略、结束占用 53 端口的进程），而「启动项」
（注册表 Run 键 / 启动文件夹）只能以普通权限拉起程序，UAC 开启时每次登录都要人工点「是」。

任务是用 **`schtasks /XML`** 建的，不是命令行参数。有四个原因：

| 事项 | 说明 |
|---|---|
| **路径含空格** | 命令行的 `/TR "…"` 要为「路径带空格」再套一层引号，schtasks 的解析扛不住，会报 `Invalid argument/option` 或 `系统找不到指定的路径`。XML 里 `<Command>` 与 `<Arguments>` 分开写，没有引号问题 |
| **72 小时限时** | 命令行方式建的任务 `ExecutionTimeLimit` 默认 **72 小时**，到点 Windows 会把进程**杀掉**。XML 里显式设 `PT0S`（不限时） |
| **电池** | 默认 `DisallowStartIfOnBatteries=true`（笔记本用电池时**根本不启动**）、`StopIfGoingOnBatteries=true`（一拔电源就停）。XML 里都设成 `false` |
| **静默启动要真正生效** | `-silent` 分支曾经提前 `return`，程序起来了却没恢复屏蔽。现在静默模式同样执行恢复流程，且**不弹任何对话框**（登录时没人在电脑前，弹窗会一直挂着） |

升级时会清掉历史遗留：换成「启动项」方案期间写过的 `Run` 键值（`ClassFirewall`）。
失败时界面和日志会带上 **schtasks 的原始输出**，不会只说一句"设置失败"。

### ⚠️ 只勾「开机自动启动」是不够的

开机自启要真正起作用，需要**三个开关一起**：

| 开关 | 为什么需要 |
|---|---|
| 勾选网站 + **启用屏蔽** | 否则没有黑名单，也没什么可恢复的 |
| **启动时自动恢复上次的屏蔽** | **少了这个，开机后程序会启动，但屏蔽仍是关着的** —— 表现就是"开机自启没有用" |
| 开机自动启动 | 建任务，登录时把程序拉起来 |

勾「开机自动启动」时如果第 2 项没勾，日志里会直接提醒。
...
| **开机自启后程序"没反应"** | 任务其实建成功了、也运行了，但程序**启动即崩溃**：任务计划里显示 `上次结果 = -1073741571` = `0xC00000FD` = **栈溢出**。根因是缩托盘时改了 `Form.ShowInTaskbar` —— 改它会**重建窗口句柄**，而重建发生在 `WmCreate` 内部，会在窗口创建途中再次触发 `Resize`；`Resize` 又调缩托盘 …… 无限递归。开机自启（`-silent`）让窗口以最小化状态创建，正好踩中最深的那条路。**加防重入标志挡不住**（重建发生在标志生效之前） | 缩托盘**只 `Hide()`，彻底不碰 `ShowInTaskbar`**（隐藏的窗口本来就没有任务栏按钮）；另加 `_formReady` 守卫，使窗口创建期间的 Resize 不动作。已用最小复现工程实测：旧写法与"仅加防重入"都崩 `0xC00000FD`，"只 Hide"的写法正常 |
| **启动崩溃悄无声息** | 栈溢出（`0xC00000FD`）在 .NET 里**无法捕获**，进程被直接终止，界面毫无提示；当时只能靠任务计划里的"上次结果"才看出来 | 加了 `%AppData%\ClassFirewall\crash.log`（能被捕获的异常都会记下）；栈溢出则只能靠消除递归来预防 |
